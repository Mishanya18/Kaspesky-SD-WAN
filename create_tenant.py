...
zabbix_proxy_name = "zabbix-proxy" # Имя zabbix proxy, с которым он добавлен в Zabbix
pnf_mgmt_ip = "192.168.200.103" # IP контроллера для коннекта к нему оркестратора
pnf_ext_ip = "192.168.200.103" # IP контроллера для коннекта к нему CPE
tenant_name = "Tenant2" # Имя создаваемого тенанта
pnf_name = "PNF for Tenant2" # Имя pnf для тенанта
domainName = "Domain" # Имя домена, в котором будет создан датацентр
dcName = "Tenant2DC" # Имя датацентра
mgmt_net_cidr = "10.11.2.0/24" # CIDR менеджмент сети
mgmt_net_start = "10.11.2.1" # первый IP для выдачи на CPE из менеджмент сети
mgmt_net_end = "10.11.2.254" # последний IP для выдачи на CPE из менеджмент сети
...
try:
    ...
    for domain in response.json()['content']:
        if domain['name'] == domainName:
            dmnID = domain['id']
    if not dmnID:
      print("Не удалось найти Domain")
